...
def splineinterpol(coefs,points,degree,flag=0,mask=None,
                   bcond=defaultbcond):
...
class MSplineSignal:
    ...
    def evalat(self,pts,mask=None):
        ...
        return splineinterpol(self.c,pts,self.degree,mask=mask)

    def evalderat(self,pts,d,mask=None):
        """ Evaluates the derivative of the signal with respect to
        the component d. Otherwise as evalat """ 
        pts=pts+self.shift # add offset 
        return splineinterpol(self.c,pts,self.degree,flag=4**d,mask=mask)
...
def test_bigsplines():
    ...
    #y=splineinterpol(c,p1,3,mask=array((0,0,1)))
